...
TASKS_CONFIG = [
 ...
 AND SUBSTR(a.VRNO,1,2)IN ('L1','SP','L2','L6')"""
    ...
    CASE WHEN B.approveddate IS NULL THEN 'UNAPPROVED' ELSE 'APPROVED' END AS APPROVE_STATUS
    ...
    FROM (
        ...
        FROM (
            SELECT
                ...
                BOM_ID,
                MAX(CASE
                        WHEN ITEM_NATURE = 'FG' THEN ITEM_CODE
                    END) OVER (PARTITION BY VRNO) AS FGITEMCODE
...
def run_sync():
    ...
    for task in TASKS_CONFIG:
        ...
        try:
            ...
            start_cell = task.get("start_cell", "A1")
            ...
            end_col_letter = colnum_string(end_col_num)

            extra_clear_rows = 5000
            clear_end_row = start_row + num_rows + extra_clear_rows

            clear_range = f"{start_col_letter}{start_row}:{end_col_letter}{clear_end_row}"

            sheet.batch_clear([clear_range])
            sheet.update(start_cell, data_to_upload)
